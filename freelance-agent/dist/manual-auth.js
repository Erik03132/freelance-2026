...
/**
 * Скрипт ручной авторизации на Kwork
 * Использует реальный Google Chrome для обхода блокировок
 */
async function manualAuth() {
    const logger = new logger_1.Logger();
    console.log('\n=== РУЧНАЯ АВТОРИЗАЦИЯ НА KWORK ===\n');
    console.log('1. Сейчас откроется браузер Google Chrome...');
    ...
    let browser = null;
    try {
        // Используем реальный Google Chrome
        ...
        const context = await browser.newContext({
            ...
            proxy: undefined, // Явно отключаем прокси
            bypassCSP: true
        });
        ...
        await page.goto('https://kwork.ru/login', { waitUntil: 'domcontentloaded', timeout: 30000 });
        ...
        await page.goto('https://kwork.ru/user', { waitUntil: 'domcontentloaded', timeout: 30000 });
        ...
        logger.error('MANUAL_AUTH', error);
        console.error('\n❌ Ошибка:', error);
        console.log('\nВозможные причины:');
        console.log('- Kwork блокирует автоматизированные браузеры');
        console.log('- Проблемы с сетью или прокси');
        console.log('- Chrome не найден или не запущен');
    }
    finally {
        if (browser) {
            await browser.close();
