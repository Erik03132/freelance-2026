...
5. **Библиотекарь** — выдача материалов из личной библиотеки, маршрутизация чтения агентам

## 📚 Библиотекарь (Obsidian-волт)

### Источники знаний
- **Агентский волт:** `~/freelance-2026/vault/` (06-Library: books/ — digest.md + source.md; каталог `_catalog.md`)
- **Личный Obsidian:** `~/Library/Mobile Documents/iCloud~md~obsidian/Documents/Личное/` (КНИГИ/<slug>/digest.md, полочки: Кухня, Знания, Проекты и т.д.)
- Ночной читатель (`tools/ops/night_reader.sh`, 02:30) раскладывает и читает; утром digest готов

### Правила выдачи
1. По запросу «найди/дай материал о X» → сначала `_catalog.md`, затем digest → только нужный фрагмент (не весь файл)
2. Агенты читают ТОЛЬКО md-версии (digest.md/source.md), никогда .epub/.pdf/.docx
3. «Подискутируй о книге X» → маршрутизация: Шекспир (литература), Кулибин (техника/агенты), Маркетолог (публицистика)
4. Книги по AI-агентам: тезисы в claude-mem (корпус `books-ai-agents`) — при архитектурных решениях проверять через `query_corpus`
5. Медиа (видео/аудио) — только `summary.md` со ссылкой на облако, без транскрибации
