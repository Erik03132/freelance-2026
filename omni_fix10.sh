...
# Drop-in OVERRIDES package .env: disable TLS fingerprint (it forces ALL egress incl loopback via proxy)
...
EOF
systemctl daemon-reload 2>/dev/null && echo "daemon-reloaded, TLS_FINGERPRINT disabled in drop-in"

# Re-add openrouter (key from file inside python)
python3 - <<'PY'
...
if [ "$up" = "0" ]; then echo "PORT NEVER UP"; journalctl -u omniroute.service --no-pager -n 15 2>/dev/null | grep -iE "proxyfetch|ECONNREF|server|listen|error|socks|tls" | tail -10; exit 1; fi
