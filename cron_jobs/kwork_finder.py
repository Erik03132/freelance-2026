...
OUR_SKILLS = {
    ...
    "automation": [r"автоматизац", r"скрипт", r"\brpa\b", r"интеграц", r"автоматизир"],
    ...
    "sql": [r"\bsql\b", r"база данн", r"postgres", r"sqlite", r"mongodb", r"nosql", r"бд\b"],
    ...
    "avito_vk": [r"avito", r"\bвк\b", r"\bvk\b", r"маркетплейс", r"telegram[- ]?канал", r"соцсет"],
    "ui_proto": [
        r"stitch",
        r"google ai studio",
        r"прототип",
...
def extract_client_info(text: str) -> dict:
    ...
    for pat in (
        r"Заказ(?:ов)?\s+на\s+сайте\s*[:\s]\s*(\d+)",
...
def evaluate(job: dict, cfg: dict | None = None) -> dict:
    ...
    if not has_real_desc:
        return _mk(
            decision="B",
            reason=(
                f"Без реального ТЗ матч недостоверен "
    ...
    final = (
        cfg["w_skill"] * s_skill
        + cfg["w_comp"] * s_comp
        + cfg["w_client"] * s_client
        + cfg["w_budget"] * s_budget
    )

    bad_client = client.get("found") and (
        client.get("orders") == 0 or (client.get("rating") is not None and client["rating"] < 3.0)
    )
    ...
    if is_A:
        decision = "A"
        reasons.append(
            f"матч {s_skill:.0f}%≥{cfg['min_skill_a']}, заявок "
    ...
    reason = "; ".join(reasons)
    return _mk(decision, reason, sm, proposals, client, budget, final=round(final, 1))
...
def _mk(decision, reason, sm, proposals, client, budget, final=None):
    ...
    return _decision(
        skill=sm["match"],
        competition=score_competition(proposals),
        client=score_client(client),
        budget=score_budget(budget),
        decision=decision,
        reason=reason,
        sm=sm,
        proposals=proposals,
...
# ---------------------------------------------------------------------------
def fetch_projects(limit_pages: int = 3, use_cookies: bool = True, delay=(30, 60)) -> list[dict]:
...
def fetch_job_detail(url: str, use_cookies: bool = True, delay=(20, 40), retries: int = 2) -> dict:
    """Догружаем страницу кворка: бюджет + описание + proposals + client.

    Kwork банит headless-chromium на уровне соединения (ERR_CONNECTION_REFUSED),
    особенно при частых заходах на карточки. Поэтому:
      - НЕ используем флаг --no-proxy-server (он сам по себе вызывает бан, см. ADR-009).
      - Делаем retries с паузой; если карточка упорно банится — возвращаем
        пустой результат (скоринг пройдёт по заголовку, без падения всего прогона).
    """
    from playwright.sync_api import sync_playwright
    ...
    for attempt in range(1, retries + 1):
        try:
            with sync_playwright() as p:
                ...
                )  # без --no-proxy-server (см. ADR-009)
                ctx = b.new_context(user_agent=rand_ua())
                if use_cookies and os.path.exists(COOKIES):
                    try:
                        with open(COOKIES) as f:
            ...
            b.close()
            break  # успешно стянули — выходим из цикла retry
        except Exception as e:
            last_err = e
            print(f"[warn] detail {url} (попытка {attempt}/{retries}): {e}", file=sys.stderr)
            time.sleep(random.uniform(*delay))
    else:
        # все попытки провалились — карточка забанена, возвращаем пусто
        if last_err:
            print(f"[warn] detail {url}: все попытки провалены ({last_err})", file=sys.stderr)
    time.sleep(random.uniform(*delay))
    return out
...
def mark_seen(url: str):
    try:
        c = sqlite3.connect(DB)
        c.execute(
            "INSERT OR IGNORE INTO jobs(platform,url,title,status) VALUES(?,?,?,?)",
            ("kwork", url, "", "new"),
        )
        c.commit()
        c.close()
...
def write_draft(job: dict, ev: dict):
    ...
    if budget:
        in_budget = (
            "в рамках бюджета заказчика"
            if our_price <= budget * 3
            else "выше желаемого, но в допустимом"
        )
        price_note = (
            f"Оценка: от {our_price:.0f} ₽ (якорь 1500 ₽/ч × ~{est_hours}ч), "
            f"{in_budget} (бюджет до {budget:.0f} ₽)."
        )
    else:
        price_note = (
            f"Цена: от {our_price:.0f} ₽ (якорь 1500 ₽/ч × ~{est_hours}ч), "
    ...
    if "telegram_bot" in matched or "aiogram" in title_low or "telegram" in title_low:
        ...
        offer_line = (
            "Готовы выполнить транскрибацию аудио/видео в текст (faster-whisper, локально)."
        )
    elif "parsing" in matched or "automation" in matched:
        offer_line = (
            "Готовы настроить парсинг/автоматизацию на Python с минимумом ручного вмешательства."
        )
    else:
        offer_line = "Готовы выполнить задачу в соответствии с ТЗ, обеспечивая качество и сроки."
    sc = ev["skill_detail"]
    skill_note = (
        f"Матч навыков: {ev['skill']:.0f}% (наши: {', '.join(sc['our']) or '—'})."
        if ev["skill"] is not None
        else "Матч навыков: неопределён"
    )
...
"""
    ...
    with open(LEADS, "a") as f:
        f.write(
            f"\n## {datetime.now(UTC).strftime('%Y-%m-%d')} | Kwork A | [{job['title']}]({job['url']})\n"
...
DEMO_JOBS = [
    ...
    },
    {
        ...
        "budget": 5000,
        "proposals": 15,
        "client_info": {"orders": 3, "rating": 4.2, "verified": False, "found": True},
    },
    {
    ...
    },
    {
        ...
        "budget": 4000,
        "proposals": 9,
        "client_info": {"orders": 1, "rating": 4.0, "verified": False, "found": True},
    },
    {
    ...
    },
    {
        ...
        "budget": 50000,
        "proposals": 5,
        "client_info": {"orders": 8, "rating": 4.6, "verified": True, "found": True},
    },
    {
...
def run_demo(cfg: dict):
    ...
    print(
        f"{'РЕШ':<4} {'SKILL':>6} {'COMP':>5} {'CLNT':>5} {'BUDG':>5} {'FIN':>6}  ЗАЯВ  ЗАКАЗЧИК  ТЗ"
    )
    for job in DEMO_JOBS:
        ...
        print(
            f"{ev['decision']:<4} {sk:>6} {ev['competition']:>5} {ev['client']:>5} "
...
def main():
    ...
    ap = argparse.ArgumentParser(description="Kwork finder v2 — многофакторный скоринг кворков")
    ...
    cfg = dict(
        min_skill_a=args.min_skill_a,
        min_skill_b=args.min_skill_b,
        max_proposals_x=args.max_proposals_x,
        max_proposals_b=args.max_proposals_b,
        min_budget=args.min_budget,
        final_threshold_a=args.final_threshold_a,
    )
    ...
    for job in jobs:
        ...
        if not title_hits(job):
            ...
            if ev["decision"] == "B":
                ...
                with open(LEADS, "a") as f:
                    f.write(
                        f"\n## {datetime.now(UTC).strftime('%Y-%m-%d')} | Kwork B | [{job['title']}]({job['url']})\n"
    ...
    print(
        f"[*] решения: A={counts['A']} B={counts['B']} X={counts['X']}, "
        f"черновиков: {drafted}" + (" (DRY-RUN, ничего не записано)" if args.dry_run else "")
    )
