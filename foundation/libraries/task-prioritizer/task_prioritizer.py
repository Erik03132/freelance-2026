...
class TaskScorer:
    ...
    def _init_default_config(self) -> None:
        ...
        # Сущности (компоненты)
        self.entities = {
            "database": EntityConfig(
                "database", EntityTier.S, ["db", "postgres", "mysql", "mongodb"]
            ),
            ...
            "api_gateway": EntityConfig(
                "api_gateway", EntityTier.S, ["gateway", "router", "endpoint"]
            ),
            ...
            "monitoring": EntityConfig(
                "monitoring", EntityTier.A, ["monitoring", "metrics", "prometheus"]
            ),
    ...
    def score_task(self, task: dict) -> ScoringResult:
        ...
        independent_signals = self._count_independent_signals(
            task, detected_triggers, detected_entities
        )
        ...
        for trigger_name, trigger_config in self.triggers.items():
            for key in trigger_config.keys:
                if self._match_key(key, text):
                    ...
                    categories[trigger_config.category].append(
                        (trigger_name, trigger_config.weight)
                    )
                    break
    ...
    def _generate_breakdown(
        self, triggers: dict, entities: dict, combos: dict, signals: int
    ) -> str:
