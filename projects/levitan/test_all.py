...
async def test_all():
    ...
    try:
        ...
        response = await llm.generate(
            [{"role": "user", "content": "Привет! Ответь кратко."}], max_tokens=50
        )
    ...
    try:
        mango = MangoClient(api_key=settings.mango.api_key, salt=settings.mango.salt)
        print("   ✅ Mango Client created")
        print("   ⚠️ Balance check skipped (network)")
        await mango.close()
    ...
    try:
        ...
        if reporter:
            print("   ✅ Telegram Reporter created")
            print("   ⚠️ Message send skipped (network)")
