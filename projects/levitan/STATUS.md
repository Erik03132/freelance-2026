# Levitan Project Status

## Текущий статус: 🟢 CRM работает (FastAPI + SQLite + веб)
...
- [x] Telegram бот @levitan_dialer_bot (новый, создан 06.07)
- [x] Автовыбор работающего прокси для Telegram
- [x] Ping Checker — проверка "живых" номеров
- [x] SIP: Zoiper подключён (ext 22)
- [x] Mango callback → полная цепочка: Zoiper → клиент
- [x] **CRM: FastAPI + SQLite + веб-интерфейс (создана 07.07)**
- [x] **CRM API интегрирован в Telegram-бота (вместо JSON)**
...
### Что сделано
1. Починено подключение Telegram-бота
2. Автовыбор работающего прокси
3. Ping Checker
4. Ответвлённые + горячие лиды
5. `/summary`, `/crm`, `/remind` команды
6. Напоминания: Белый (27 июл), Виктор (10 июл), Георгиевское (15 июл)
7. Webhook сервер `scripts/webhook_server.py`
8. CRM запроектирована: создана структура `crm/`

## Сессия 2026-07-07

### Что сделано
1. **CRM — полный код:** FastAPI + SQLite (aiosqlite) + веб-интерфейс (Jinja2)
   - Дашборд со статистикой (всего контактов, лидов, звонков сегодня, напоминаний)
   - Список контактов с поиском и фильтрацией по статусу
   - Карточка контакта с деталями, транскриптом, быстрыми действиями
   - Страница напоминаний с отметкой выполненных
   - Импорт из JSON-файлов (существующие данные)
   - REST API: `/api/contacts`, `/api/reminders`, `/api/stats`, `/api/import`
2. **Бот интегрирован с CRM API:**
   - `save_to_crm()` дублирует данные в CRM через POST `/api/contacts`
   - `/crm` команда читает данные через CRM API (с fallback на локальный JSON)
   - `/remind list/del` через CRM API (с fallback)
   - `_check_reminders` использует CRM API
3. **Импортированы существующие данные:** 30 контактов, 3 напоминания
4. **Архитектура:** порт 8088, `uvicorn crm.app:app`

### Файлы
| Файл | Назначение |
|------|-----------|
| `crm/database.py` | aiosqlite, init_db, get_db |
| `crm/app.py` | FastAPI: 21 route (10 web + 11 API) |
| `crm/templates/*.html` | 5 Jinja2 шаблонов |
| `crm/static/style.css` | CSS (дашборд, карточки, таблицы) |
| `crm/crm.db` | SQLite БД (30 contacts, 3 reminders) |

### Запуск
```bash
# CRM (порт 8088)
source .venv/bin/activate
python3 -m uvicorn crm.app:app --host 0.0.0.0 --port 8088
...
### Следующие шаги
1. **Развернуть CRM на VPS** — когда появится доступ
2. **Настроить Mango webhook** → конспекты летят на CRM API
3. **Продолжить обзвон** — 🎠 Карусель
4. **Greeting Bridge (baresip)** — приветствие перед соединением
5. **Улучшить CRUD в вебе** — редактирование контактов, удаление
