    ...
    try:
        ok, _ = run_cmd(
            [
                ...
                str(path),
                "-vf",
                "scale=256:256:force_original_aspect_ratio=decrease,edgedetect=low=0.05:high=0.3,format=gray",
                "-frames:v",
                "1",
                "-f",
                "rawvideo",
...
def generate_grain_overlay(duration_s):
    ...
    ok, _ = run_cmd(
        [
            ...
            "lavfi",
            "-i",
            f"nullsrc=s={TARGET_W}x{TARGET_H}:d={frames}:r={FPS}",
            "-vf",
            "geq=lum='random(1)*20+8':cb=128:cr=128,format=yuv420p",
            "-c:v",
            "libx264",
            "-preset",
            "ultrafast",
