...
class LLM2VecEmbedder:
    ...
    def encode(
        self, texts: str | list[str], batch_size: int = 8, normalize: bool = True
    ) -> np.ndarray:
        """
        ...
        for i in range(0, len(texts), batch_size):
            ...
            with torch.no_grad():
                batch_embeddings = self.model.encode(
                    batch, max_length=self.max_length, normalize=normalize
                )
...
def encode(texts: str | list[str], device: str = "cpu") -> np.ndarray:
