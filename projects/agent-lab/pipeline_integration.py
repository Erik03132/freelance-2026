...
class ContentCombinePipeline:
    ...
    def __init__(self, config_path: str):
        ...
        if self.config.get("telegram_bot_token"):
            self.telegram_exporter = TelegramExporter(self.config["telegram_bot_token"])
    ...
    def step6_generate_outputs(
        self, content: list[dict], clusters: list[dict], min_score: int = 50
    ) -> dict:
        ...
        print(
            f"[pipeline] Generated: {len(alerts)} alerts, {len(digest)} digest, {len(trending)} trending"
        )
    ...
    async def step7_export(self, outputs: dict) -> int:
        ...
        stats = {
            ...
            "avg_score": sum(item.get("score", 0) for item in all_items) / len(all_items)
            if all_items
            else 0,
        ...
        sent = await self.telegram_exporter.export_pipeline_results(
            chat_id, outputs["alerts"], outputs["digest"], outputs["trending"], stats
        )
    ...
    async def run(self):
        ...
        try:
            ...
            outputs = self.step6_generate_outputs(
                content, clusters, min_score=self.config.get("min_score", 50)
            )
