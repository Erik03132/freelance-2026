...
def normalize():
    ...
    try:
        ...
        return jsonify({"status": "success", "count": len(content_dicts), "content": content_dicts})
...
def normalize_twitter():
    ...
    try:
        ...
        if not bearer_token:
            return jsonify({"status": "error", "message": "bearer_token required"}), 400
...
def normalize_telegram():
    ...
    try:
        ...
        if not bot_token or not channel_id:
            return jsonify({"status": "error", "message": "bot_token and channel_id required"}), 400
