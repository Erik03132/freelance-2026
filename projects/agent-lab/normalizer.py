...
class Normalizer:
    ...
    def normalize_habr_html(
        self, page_url: str = "https://habr.com/ru/articles/"
    ...
    def _parse_habr_html_article(self, article_elem) -> NormalizedContent | None:
        ...
        try:
            ...
            title = (
                title_span.get_text(strip=True) if title_span else title_link.get_text(strip=True)
            )
            ...
            published_at = (
                time_elem.get("datetime", datetime.now(UTC).isoformat())
                if time_elem
    ...
    def normalize_habr_rss(
        self, feed_url: str = "https://habr.com/ru/feed/"
    ) -> list[NormalizedContent]:
    ...
    def _parse_habr_entry(self, entry: dict) -> NormalizedContent | None:
    ...
    def normalize_habr_api(self, article_ids: list[str] | None = None) -> list[NormalizedContent]:
    ...
    def _parse_habr_api_article(self, article: dict) -> NormalizedContent | None:
        ...
        try:
            ...
            published_at = article.get("published_at", datetime.now(UTC).isoformat())
    ...
    def _parse_medium_entry(self, entry: dict) -> NormalizedContent | None:
    ...
    def _parse_twitter_tweet(self, tweet: dict, users_map: dict) -> NormalizedContent | None:
        """Parse Twitter tweet"""
        try:
            ...
            published_at = tweet.get("created_at", datetime.now(UTC).isoformat())
    ...
    def normalize_telegram_channel(
        self, channel_id: str, bot_token: str, limit: int = 50
    ) -> list[NormalizedContent]:
    ...
    def _parse_telegram_message(self, message: dict, channel_id: str) -> NormalizedContent | None:
        ...
        try:
            ...
            published_at = datetime.fromtimestamp(message.get("date", 0), tz=UTC).isoformat()
