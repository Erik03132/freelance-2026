"""
Тест Sentence-Transformers эмбеддера.

Проверяет:
1. Загрузку модели
2. Генерацию эмбеддингов
3. Семантический поиск
4. Сравнение с keyword search
"""
...
def test_basic():
    ...
    start = time.time()
    embedder = SentenceTransformerEmbedder(model_name="all-MiniLM-L6-v2", device="cpu")
    ...
    print(f"⏱️  Время кодирования {len(texts)} текстов: {encode_time:.3f}s")
...
def test_similarity(embedder):
    ...
    print(f"\n🔍 Запрос: '{query}'")
    print(f"⏱️  Время поиска: {search_time:.3f}s")
...
def test_comparison(embedder):
    ...
    st_scores = embedder.similarity(query, documents)
    ...
    print(f"\n🔍 Запрос: '{query}'")
    print("\n📊 Сравнение методов:")
    print(f"{'Документ':<50} {'Semantic':>10} {'Keyword':>10}")
    print("-" * 72)

    for doc, st_s, kw_s in zip(documents, st_scores, keyword_scores):
        doc_short = doc[:47] + "..." if len(doc) > 50 else doc
        print(f"{doc_short:<50} {st_s:>10.4f} {kw_s:>10}")

    print("\n💡 Вывод: Semantic search понимает смысл, keyword — только слова")


if __name__ == "__main__":
    print("🚀 Тестирование Sentence-Transformers Embedder\n")
