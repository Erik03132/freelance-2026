...
class PexelsFetcher:
    ...
    async def search_photo(self, query: str, per_page: int = 1) -> dict | None:
...
class TelegramExporter:
    def __init__(self, bot_token: str, pexels_api_key: str | None = None):
    ...
    def format_alerts(self, alerts: list[dict], chat_id: str) -> TelegramMessage:
        ...
        for i, item in enumerate(top5, 1):
            ...
            text += (
                f"   Score: {item.get('score', 0)} | Priority: {item.get('priority', 'unknown')}\n"
            )
        ...
        return TelegramMessage(chat_id=chat_id, text=text, parse_mode="HTML")
    ...
    def format_digest(self, digest: list[dict], chat_id: str) -> TelegramMessage:
        ...
        if items_with_photos:
            ...
            return TelegramMessage(
                chat_id=chat_id, text=text, parse_mode="HTML", photo_url=photo_url
            )
        else:
            # Send text only
            text = "📰 <b>DIGEST</b>\n\n"
