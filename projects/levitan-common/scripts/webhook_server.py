...
def handle_call_state(data: dict):
    ...
    _write_event(
        {"type": "call_state", "state": call_state, "call_id": call_id, "entry_id": entry_id}
    )
...
def handle_summary(data: dict):
    ...
    phone = (
        ...
        or data.get("from", {}).get("number", "")
        or data.get("to", {}).get("number", "")
        or ""
    )
...
async def mango_webhook(request: Request):
    ...
    has_summary_event = "summary" in (data.get("type", "").lower()) or request.url.path.endswith(
        "/summary"
    )
