...
if u22:
    for u in u22 if isinstance(u22, list) else [u22]:
        print(
            f"   ID:22 = {u.get('NAME','')} {u.get('LAST_NAME','')} | Active: {u.get('ACTIVE')} | Pos: {u.get('WORK_POSITION','')}"
        )
...
if u1586:
    for u in u1586 if isinstance(u1586, list) else [u1586]:
        print(
            f"   ID:1586 = {u.get('NAME','')} {u.get('LAST_NAME','')} | Active: {u.get('ACTIVE')} | Pos: {u.get('WORK_POSITION','')}"
        )
...
for name, dlist in sorted(
    mgr_deals.items(),
    key=lambda x: -sum(float(d.get("OPPORTUNITY", 0) or 0) for d in x[1]),
):
