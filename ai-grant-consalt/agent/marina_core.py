...
if os.path.exists(NAVIGATOR_PATH):
    with open(NAVIGATOR_PATH, encoding="utf-8") as f:
        _navigator = f.read()
...
marina_model = genai.GenerativeModel("gemini-2.0-flash", system_instruction=MARINA_SYSTEM_PROMPT)
    ...
    try:
        ...
        response = pplx_client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a legal researcher. Find the most up to date russian laws, tax regulations, and grants regarding the user query. Always cite sources with article numbers.",
                },
        ...
        return "Perplexity временно недоступен."
...
def fetch_local_context(query: str) -> str:
    ...
    print("📚 [Vector Search]: Ищу в Neon DB...")
    try:
...
"""
            ...
            resp = requests.post(
                ...
                json={
                    ...
                    "messages": [
                        ...
                        {
                            "role": "user",
                            "content": f"Контекст: {local_context[:2000]}\n\nВопрос: {query}",
                        },
                    ],
                    "temperature": 0.3,
                },
    ...
    return (
        "К сожалению, все AI-модели временно недоступны. Попробуйте повторить вопрос через минуту."
    )
...
def main_rag_pipeline(query: str, history=None):
    ...
    if graph_result.get("total_nodes", 0) > 0:
        print(
            f"   ✅ Найдено: {len(graph_result['primary_nodes'])} прямых + {len(graph_result['linked_nodes'])} связанных узлов"
        )
    else:
