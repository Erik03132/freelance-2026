...
def get_products_info(query):
    try:
        ...
        cur.execute(
            "SELECT name, price, stock_status, description FROM products WHERE name ILIKE %s LIMIT 3",
            (f"%{query}%",),
        )
        rows = cur.fetchall()
        cur.close()
...
async def start_handler(message: types.Message):
    await message.answer(
        "Привет! Я Анжелочка 🐣 Чем я могу вам помочь? У нас есть отличные инкубационные яйца и корма!"
    )
