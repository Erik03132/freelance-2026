...
# Force dev mode so Next.js does NOT block listen on self-healthcheck
...
EOF
systemctl daemon-reload 2>/dev/null && echo "daemon-reloaded, NODE_ENV=development"
...
if [ "$up" = "0" ]; then echo "PORT NEVER UP"; journalctl -u omniroute.service --no-pager -n 15 2>/dev/null | grep -iE "proxyfetch|ECONNREF|server|listen|error|base|node_env" | tail -10; exit 1; fi
