# 🌙 Ночной аудит кода — 2026-05-30

> **Проект:** AI-Eggs (Анжелочка)  
> **Время:** 02:00:05  
> **Метод:** Cross-Model Peer Review  
> **Режим:** 🔧 AUTO-FIX  
> **Python файлов:** 197 (проверяем: 5)  
...
ai-eggs/agent/_vk_get_token.py:3:1: E401 [*] Multiple imports on one line
ai-eggs/agent/_vk_get_token.py:22:17: S105 Possible hardcoded password assigned to: "client_secret"
ai-eggs/agent/_vk_get_token.py:27:5: S603 `subprocess` call: check for execution of untrusted input
ai-eggs/agent/_vk_get_token.py:39:89: E501 Line too long (101 > 88)
ai-eggs/agent/_vk_photo_workaround.py:3:1: E401 [*] Multiple imports on one line
ai-eggs/agent/_vk_photo_workaround.py:20:89: E501 Line too long (126 > 88)
ai-eggs/agent/_vk_photo_workaround.py:30:9: S603 `subprocess` call: check for execution of untrusted input
...
🔧 **ruff --fix:** 2 ошибок исправлено автоматически (ветка: \'auto-fix/night-audit-2026-05-30\')

**Критических ошибок ruff (E,F,S,B):** 1661
...
```
 ai-eggs                                   |  0
 checkpoints/chp_20260529_230005.md        | 25 ++++++++++
 reports/night_audit_ai-eggs_2026-05-30.md | 79 +++++++++++++++++++++++++++++++
...
**angelochka_core.py:16**  
```python
import re
import re as _re_core
```
**Критичность:** 🟢 Минорно  
**Исправление:** оставьте только `import re` и используйте один и тот же модуль, дубль импорта лишний.

---

**angelochka_core.py:52–55**  
```python
        _cfg = json.load(_f)
    user_entry = _cfg.get("users", {}).get(sid, {})
```
**Критичность:** 🟡 Важно  
**Исправление:** в блоке `except json.JSONDecodeError` переменную `_cfg` нельзя использовать — она не существует. Инициализируйте `_cfg = {}` до `try`.

---

**angelochka_core.py:66**  
```python
...
```
**Критичность:** 🟢 Минорно  
**Исправление:** переменная `_e` больше нигде не используется. Достаточно `except Exception:` без присваивания, либо переиспользуйте `e`.

---

**tg_bot.py:13**  
`user_histories` глобальный dict-сторадж  
**Критичность:** 🔴 Критично  
**Исправление:** при горизонтальном масштабировании контейнеров/ко-воркеров значения будут теряться. Перенесите историю в Redis, PostgreSQL или любое другое централизованное хранилище.
...
```
**Критичность:** 🔴 Критично  
**Исправление:** жёстко прописан ID. Падает сразу, если `.env` не задан. Ошибку лучше формировать без демонстрации реального ID.

---

**tg_bot.py:118-126**  
Лишний вырезок кода (`--- truncated ---`) в диффе  
**Критичность:** 🟢 Минорно  
**Исправление:** проверьте, что файл не содержит строки-заглушки вида `--- (truncated) ---`.

---

**bitrix_intelligence.py:18**  
```python
sys.path.insert(0, AGENT_DIR)
```
**Критичность:** 🟡 Важно  
**Исправление:** после `sys.path.insert` не выполняется `remove`. При ре-импортировании модуля или релоаде может появиться дубль пути и странный импорт. Заверните в `try/finally` или используйте `PYTHONPATH`.

---

**bitrix_intelligence.py:26**  
`requests.post(..., timeout=20)`  
**Критичность:** 🟢 Минорно  
**Исправление:** если скрипт запускается внутри async-окружения (например, из `chat_listener`), синхронный `requests` блокирует event-loop. Лучше применять `httpx.AsyncClient` или `aiohttp`.

---

**bitrix_intelligence.py:51**  
```python
        items = items.get("tasks", items.get("items", []))
```
**Критичность:** 🟢 Минорно  
**Исправление:** приводит `items` к списку, но если объект содержит другие ключи или неожиданную структуру, может вернуть пустой список, не логгируя проблему. Добавьте валидацию и лог приводимых ключей.

---

**bitrix_intelligence.py:41**  
```python
max_iterations = 50  # защита от бесконечного цикла при сбое API
```
**Критичность:** 🟢 Минорно  
**Исправление:** при `next_page >= limit` (например, `limit = 200`, а `start = 0`) остаётся 199, а `max_iterations = 50` вполне может не хватить — проверьте реальное поведение API. Лучше увеличить или убрать верхний предел, полагаясь на `next`.

---

### 📌 Итог

✅ Никакого «мертвого» или закомментированного кода не найдено.  
Найдены реальные проблемы: race-условушка хранения истории, отсутствие инициализации переменной `_cfg`, жёстко зашитый ID и blocks-вызовы requests внутри asyncio.
...
|---------|----------|
| 📅 Дата | 2026-05-30 |
| ⏰ Время | 02:00:05 → 02:00:54 |
| 📁 Python файлов | 197 |
| 📝 Изменено за день | 3 |
| ⚡ ruff ошибок (E,F,S,B) | 1661 |
