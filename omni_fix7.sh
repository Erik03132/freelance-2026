...
# 2. drop-in: UNSET proxy env (override systemd unit's HTTPS_PROXY) so internal loopback works
mkdir -p /etc/systemd/system/omniroute.service.d
cat > /etc/systemd/system/omniroute.service.d/override.conf <<'EOF'
[Service]
TimeoutStartSec=300
RestartSec=10
# Unset global proxy so OmniRoute internal loopback (127.0.0.1:20128) is NOT routed via proxy
Environment=HTTP_PROXY=
Environment=HTTPS_PROXY=
Environment=NO_PROXY=
EOF
systemctl daemon-reload 2>/dev/null && echo "daemon-reloaded, proxy unset in drop-in"

# 3. re-add openrouter to DB (with created_at to satisfy NOT NULL)
...
OR_KEY = open("/root/.openrouter_key.bak").read().strip() if False else None
PY
python3 /root/add_openrouter.py "$OR_KEY" 2>/dev/null || python3 - <<PY
import sqlite3, datetime
db="/root/.omniroute/storage.sqlite"
import sys
OR_KEY = sys.argv[1]
...
if not c.fetchone():
    ...
    print("openrouter already present")
...
# 4. start only systemd (proxy now unset)
...
if [ "$up" = "0" ]; then
  echo "PORT NEVER UP"; journalctl -u omniroute.service --no-pager -n 20 2>/dev/null | grep -iE "proxyfetch|ECONNREF|server|listen|error" | tail -12; exit 1
fi
