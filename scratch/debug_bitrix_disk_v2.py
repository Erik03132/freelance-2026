...
def test_disk():
    ...
    webhook_url = (
        os.getenv("SANDBOX_BITRIX_WEBHOOK_URL")
        or "https://b24-mjxvhq.bitrix24.ru/rest/1/lqx5kngqbh7k09nx/"
    )
    ...
    try:
        ...
        for s in storages:
            print(
                f"ID: {s['ID']}, Name: {s['NAME']}, Entity: {s['ENTITY_TYPE']}, EntityID: {s['ENTITY_ID']}"
            )
    ...
    try:
        res = requests.get(
            f"{webhook_url}/disk.storage.getchildren.json", params={"id": storage_id}
        )
        ...
        if not found_root:
            ...
            res = requests.post(
                f"{webhook_url}/disk.storage.addfolder.json",
                json={"id": storage_id, "data": {"NAME": "00_CONTENT_MACHINE_EGGS"}},
            )
