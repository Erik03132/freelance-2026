...
- [ ] **MM-3.** Завести DASHSCOPE_API_KEY (Alibaba Cloud DashScope) + SERPER_API_KEY, если появится мультимодальная задача (OCR чеков/скринов, верификация скриншотов, видео-аналитика)
...
- [ ] **MA-1.** Изучить Toolport (github) как единый порт для наших MCP-серверов: сравнить с текущим `mcp` в opencode.jsonc, измерить экономию токенов от per-task включения тулов. Ставить только если реальный выигрыш на метрике
...
- [ ] **MA-3.** BrowserOS neo — держать в справочнике как кандидата под GUI-агентов: если появится задача «верификация скриншотов/UI-тесты», брать его или Qwen-MM (MM-1) по принципу «меньше кода»
...
- [ ] **OB-1.** [проверено (src) → ПРИНЯТО, с оговоркой] YAML frontmatter в vault-заметках (минимум: `type`, `tags`, `status`, `updated`). ВЕРИФИЦИРОВАНО: James Croft / Nick Liu / Blake Crosley (Obsidian MCP Guide) / OKF (Google Cloud spec, required `type`) + OSS `mcp-obsidian`, `mcpvault`. Frontmatter = queryable data, агент фильтрует без чтения всех файлов. ОГОВОРКА: не серебряная пуля — нужны ещё vault/CLAUDE.md-rulebook + шаблоны + retrieval (MCP); 5 полей > 20. Просить агента самого предлагать свойства — подтверждено (dsebastien: «agents fill/fix/validate properties»). Связь с AGENTS.md 0.1
- [ ] **OB-2.** Иерархия inbox(свалка/мок)→карта/заметки — сопоставить с нашим CONTEXT.md/ACTIVE_TASKS flow. Если нет явного inbox — завести vault/00-Inbox/ для сырья перед обработкой в 06-Library
...
- [ ] **SM-2.** Оркестрация ролей (PO/архитектор/разработчик/QA) — сопоставить с нашей моделью: igorek (оркестратор) + спецагенты (artemiy/rembrandt/botman/shakespeare/...) + кодеры. Зафиксировать как подтверждение архитектуры, проверить что роли QA/критика явно назначены в типовом флоу
- [ ] **SM-3.** «Фундамент до агентов» (доска/IDE/runtime/data/MCP) — сверить наш стек: доска (ACTIVE_TASKS), runtime (VPS/pm2), data (claude-mem), MCP (4 сервера). Если gap нет — закрыть как check; low-code-часть статьи (erStudio) отклонена как вендорский питч
