...
# Drop-in OVERRIDES package .env (highest priority): disable SOCKS5, no global proxy, protect loopback
...
EOF
systemctl daemon-reload 2>/dev/null && echo "daemon-reloaded, SOCKS5 disabled in drop-in"

# Re-add openrouter (key read from file inside python, not argv)
python3 - <<'PY'
...
if [ "$up" = "0" ]; then echo "PORT NEVER UP"; journalctl -u omniroute.service --no-pager -n 15 2>/dev/null | grep -iE "proxyfetch|ECONNREF|server|listen|error|socks" | tail -10; exit 1; fi
