...
Команды меню (Telegram Bot API не допускает пробел в команде):
  ...
  /use_profile_chief  — маршрутизатор (знает 6 спецов)
  /use_profile_batrak / sherlock / femida / defender / marketer / financier / health

ВАЖНО: команды переключения — ТОЛЬКО вида /use_profile_<id>. Голые
/sherlock, /femida и т.п. бот НЕ распознаёт как свитч (это пойдёт как
обычное сообщение через активный профиль). Для быстрого свитча используй
кнопки клавиатуры «👤 <Спец>» или пиши /use_profile_<id>.
...
TELEGRAM_TOKEN = os.getenv("HERMES03132_BOT_TOKEN")
if not TELEGRAM_TOKEN:
    raise ValueError("❌ HERMES03132_BOT_TOKEN не найден в .env!")

ADMIN_ID = int(os.getenv("HERMES_ADMIN_ID", "176203333"))
...
PROFILES = {
    "chief": ("use_profile_chief", "🧠 Chief (маршрутизатор)"),
...
def _pid_from_text(text: str) -> str | None:
    ...
    # текст кнопки "👤 🤖 Hermes (Батрак)" → ищем по title
...
async def cmd_start(message: types.Message):
    ...
    await message.answer(
        "🤖 <b>Hermes</b> — маршрутизатор к спецам Hermes.\n\n"
...
async def main():
    if not _acquire_lock():
        return
    print("\n🤖 Hermes — маршрутизатор к Hermes-спецам")
