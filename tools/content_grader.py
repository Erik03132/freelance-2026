...
def grade_eeat(text: str) -> RubricResult:
    ...
    grades.append(
        GradeResult(
            ...
            suggestions=["Добавь конкретные цифры: результаты, метрики, сроки"]
    ...
    grades.append(
        GradeResult(
            ...
            suggestions=["Добавь прямой ответ 40-60 слов на главный вопрос"]
            if extract_score < 4
            else [],
        )
    )
...
def grade_human_first(text: str) -> RubricResult:
    ...
    grades.append(
        GradeResult(
            ...
            suggestions=["Чередуй короткие фразы (3-7 слов) с длинными (20+ слов)"]
            if burst_score < 6
            else [],
        )
    )
    ...
    grades.append(
        GradeResult(
            ...
            suggestions=["Добавь личное мнение или прямое обращение к читателю"]
            if voice_score < 4
            else [],
        )
    )
    ...
    passive_markers = re.findall(
        r"\b(был[аоие]?\s+\w+[аоие]н[аоие]?|является|осуществляется|производится)\b",
        text,
        re.I,
    )
    ...
    grades.append(
        GradeResult(
            ...
            suggestions=["Перепиши пассивные конструкции в активный залог"]
            if active_score < 6
            else [],
        )
    )
...
def grade_vk_farm(text: str) -> RubricResult:
    ...
    has_hook = bool(
        re.search(r"[?!]|^\d|\b(как|почему|зачем|а вы знали|секрет|ошибк)", first_lines, re.I)
    )
    ...
    cta_markers = re.findall(
        r"(а у вас|делитесь|расскажите|напишите|поставьте класс|лайк|комментари|\?$)",
        text,
        re.I | re.M,
    )
    ...
    grades.append(
        GradeResult(
            ...
            suggestions=["Добавь вопрос в комментарии: «А у вас как?»"]
            if len(cta_markers) == 0
            else [],
        )
    )
    ...
    grades.append(
        GradeResult(
            ...
            suggestions=[
                f"Замени {e} → см. SKILL.md (разрешённые эмодзи)" for e in banned_found[:3]
            ],
        )
    )

    # 5. Тёплый тон (не канцелярит)
    warm_markers = re.findall(
        r"(😀|😊|🔥|👍|❤|😁|🐔|🐣|🌱|ахах|хех|ну|блин|классн|круто)", text, re.I
    )
    ...
    grades.append(
        GradeResult(
            ...
            suggestions=["Убери канцеляризмы, добавь эмодзи и живой язык"]
            if warm_score < 5
            else [],
        )
    )
