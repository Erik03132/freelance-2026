...
def extract_skill_info(skill_dir: Path) -> SkillInfo | None:
...
def extract_triggers(name: str, description: str, content: str) -> list:
    ...
    for match in re.findall(
        r'(?:when|use when|Also use when)[^.]*?["\']([^"\']+)["\']', description, re.I
    ):
        triggers.add(match.lower())
    ...
    first_50 = "\n".join(content.split("\n")[:50])
    for match in re.findall(
        r"(?:триггер|trigger|использу|use when|когда)[^.]*?:\s*(.+?)$",
        first_50,
        re.M | re.I,
    ):
...
def categorize_skill(name: str, description: str) -> str:
    ...
    if any(w in combined for w in ["seo", "marketing", "content", "copy", "social", "brand"]):
        return "marketing"
    if any(
        w in combined for w in ["engineer", "debug", "deploy", "code", "frontend", "backend", "bot"]
    ):
        return "engineering"
...
def generate_manifest(skills: list) -> str:
    ...
    for cat in cat_order:
        ...
        for s in skills_in_cat:
            ...
            lines.append(
                f"| **{s.name}** {extra_str} | {desc_short} | {size_str} | {triggers_str} |"
            )
...
def main():
    ...
    if args.format == "json":
        ...
        print(
            f"📊 {len(skills)} скиллов | ~{sum(s.est_tokens for s in skills):,} → ~{len(skills) * 50} токенов"
        )
