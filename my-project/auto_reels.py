...
def process_to_916(clip, target_res=TARGET_RES):
    ...
    bg = bg.set_position("center").crop(
        x_center=bg.w / 2, y_center=bg.h / 2, width=target_w, height=target_h
    )
    bg = bg.fl_image(
        lambda image: np.array(Image.fromarray(image).filter(ImageFilter.GaussianBlur(radius=20)))
    )
