...
# Drop-in: empty BASE_URL so no self-fetch to localhost:20128 at startup; proxy disabled; loopback protected
...
EOF
systemctl daemon-reload 2>/dev/null && echo "daemon-reloaded, BASE_URL emptied in drop-in"
...
if [ "$up" = "0" ]; then echo "PORT NEVER UP"; journalctl -u omniroute.service --no-pager -n 15 2>/dev/null | grep -iE "proxyfetch|ECONNREF|server|listen|error|base" | tail -10; exit 1; fi
